...
const FilterComposerPanel: React.FC<FilterComposerPanelProps> = ({ ideaName }) => {
  const name = ideaName ?? 'no idea selected';
  const tokens = useUiScopedTokens(['global', 'page:lab', 'region:lab:filter']);
    ...
    >
      ...
      <div className="px-3 py-2 border-b border-slate-800 bg-slate-900/70 flex items-center justify-between">
        ...
        <span className="text-[10px] text-slate-500">
          Candidate engine for: <span className="font-semibold text-slate-200">{name}</span>
        </span>
      </div>
      ...
      <div className="px-4 py-3 text-xs text-slate-300 space-y-2">
        ...
        <p className="text-slate-400">Planned features:</p>
        ...
        <p className="text-[11px] text-slate-500">
          Also a stub for now – we&apos;re just carving out the layout so the Lab has clear “where
          things live” zones.
        </p>
      </div>
    </section>
